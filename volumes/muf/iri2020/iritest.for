...
c You can turn off (jf(..)=.false.) the computation of certain  
c parameters if you do not need these parameters:
...
1       print *,'jmag(=0/1,geog/geom),lati/deg,long/deg'
     ...
     &                        'valley, width and depth,)'
        ...
        print *,'upper boundary in km for TEC integration',
     &		' (0 for no TEC)'
        read(5,*) htec_max
        if(htec_max.gt.0) then
		  print *,'lower boundary in km for TEC integration'
          read(5,*) htec_min
		  endif
...
c          jf(29)=.false.     ! t=old  f=New Topside options (f)
...
c --------- user input: B1            
            ...
            i=i+1
            var=var+vstp
            if(ivar.gt.1.and.i.le.numstpp) goto 2234
         endif
       endif
...
c       output.txt
        ...
        phour=hour
        call iri_web(jmag,jf,xlat,xlon,iy,mmdd,iut,hour,hxx
     &     ,htec_min,htec_max,ivar,vbeg,vend,vstp,outf,oar)
...
3914    format(/'TEC [1.E16 m-2] is obtained by numerical integ',
     &     'ration in 1km steps'/'  from 50 to ',f7.1,' km. ',
...
8193  FORMAT(/'-'/A7,' ELECTRON DENSITY    TEMPERATURES',
     &  6X,'ION PERCENTAGES[%]*10',5X,'1E16m-2'/1X,A6,3X,'Ne/cm-3 Ne/',
...
c		print*,xcor,tec,itopp,xne,xner,oar(1,li)/1.e6
        WRITE(7,7117) XCOR,xne,xner,jtn,jti,jte,jio,jin,
     &        jih,jihe,jio2,jino,jicl,tec,itopp
c7117    FORMAT(F7.1,1PE9.2,1x,F6.3,3I6,7I4,f6.1,i4)
7117    FORMAT(F7.1,E11.4,F6.3,3I6,7I4,f6.1,i4)
